...
import { BCRYPT_ROUNDS } from '@truck-shipping/shared-utils';
...
export class AuthService {
  ...
  async register(input: RegisterInput): Promise<AuthTokens> {
    ...
    const user = await this.userRepository.create({
      ...
      ...(input.role === 'SHIPPER' && { shipperProfile: { create: {} } }),
      ...(input.role === 'CARRIER' && { carrierProfile: { create: {} } }),
    });
  ...
  async login(input: LoginInput): Promise<AuthTokens> {
    ...
    if (!user) {
      // Timing-safe: hash even when user not found
